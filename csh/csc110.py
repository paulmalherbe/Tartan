"""
SYNOPSIS
    Cash A nalysis File Maintenance.
...
class csc110(object):
    ...
    def setVariables(self):
        ...
        if self.sql.error:
            return
        self.acc = self.sql.getRec("cshctl", where=[("ccc_cono", "=",
            self.opts["conum"])], limit=1)
        if not self.acc:
            self.new = True
            self.acc = [self.opts["conum"], "N", "", ""]
        else:
            self.new = False
        if self.genleg:
    ...
    def drawDialog(self):
        ...
        if self.genleg:
            ...
            seq = 0
        fld.extend([
            (("T",0,seq,0),"ITX",50,"Email Address","",
                self.acc[2],"N",None,None,None,("email",))])
    ...
    def doEnd(self):
        ...
        for x, d in enumerate(self.df.t_work[0][0]):
            if self.genleg and x < 3:
                ...
                data.append(d)
        if self.new:
            self.sql.insRec("cshctl", data=data)
        elif data != self.acc[:len(data)]:
            col = self.sql.cshctl_col
            data.append(self.acc[col.index("ccc_xflag")])
            self.sql.updRec("cshctl", data=data, where=[("ccc_cono",
                "=", self.opts["conum"])])
            dte = int("%04i%02i%02i%02i%02i%02i" % time.localtime()[:-3])
            for num, dat in enumerate(self.acc):
                if dat != data[num]:
                    self.sql.insRec("chglog", data=["cshctl", "U",
