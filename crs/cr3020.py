...
class cr3020(object):
    ...
    def setVariables(self):
        ...
        self.gvt = [0] * (len(crtrtp) + 1)
        self.totind = "N"
        return True

    def mainProcess(self):
        self.tit = ("%03i %s" % (self.opts["conum"], self.opts["conam"]),
            "Creditor's Ledger Audit Trail (%s)" % self.__class__.__name__)
        data = ["All Types"]
        for typ in crtrtp:
            data.append(typ[1])
        btt = {
        ...
        btm = {
            ...
            "cols": (
                ("crt_batch", "", 0, "Bat-Num"),
                ("crt_type", ("xx", crtrtp), 20, "Type"),
                ("crt_curdt", "", 0, "Cur-Dat")),
            "where": [],
            "group": "crt_batch, crt_type, crt_curdt",
    ...
    def doBatTyp(self, frt, pag, r, c, p, i, w):
        ...
        self.btyp = w
        self.whr = [("crt_cono", "=", self.opts["conum"])]
        if self.ptyp == "F":
            self.whr.append(("crt_curdt", "between", self.sperw, self.eperw))
        else:
            self.whr.append(("crt_capdt", "between", self.sdatw, self.edatw))
        if self.btyp:
            self.whr.append(("crt_type", "=", self.btyp))
        self.df.topf[pag][i+1][8]["where"] = self.whr

    def doBatNum(self, frt, pag, r, c, p, i, w):
        self.batch = w
        if self.batch:
            self.whr.append(("crt_batch", "=", self.batch))
    ...
    def doEnd(self):
        ...
        recs = self.sql.getRec("crstrn", join=jon, cols=col,
    ...
    def exportReport(self, recs):
        ...
        for num, dat in enumerate(recs):
            ...
            acno, trdt, trtp, ref1, batch, debit, credit, taxamt, detail, \
                taxind, name = vals
            line = ["BODY", [trtp.work, batch.work, acno.work, name.work,
                ref1.work, trdt.work, debit.work, credit.work, taxamt.work,
    ...
    def printReport(self, recs):
        ...
        for num, dat in enumerate(recs):
            ...
            if self.pglin > self.fpdf.lpp:
                self.pageHeading()
            if self.totsonly != "Y":
                self.fpdf.drawText("%s %s %s %s %s %s %s %s %s" % (acno.disp,
                    name.disp, ref1.disp, trdt.disp, debit.disp, credit.disp,
    ...
    def getValues(self, data):
        ...
        ref1 = CCD(data[3], "Na", 9)
        batch = CCD(data[4], "Na", 7)
        if data[5] < 0:
            debit = CCD(0, "SD", 15.2)
            credit = CCD(data[5], "SD", 15.2)
        else:
            debit = CCD(data[5], "SD", 15.2)
            credit = CCD(0, "SD", 15.2)
        taxamt = CCD(data[6], "SD", 15.2)
        detail = CCD(data[7], "NA", 30)
        taxind = CCD(data[8], "NA", 1)
        name = CCD(data[9], "NA", 30)
        ...
        self.gvt[trtp.work - 1] = float(ASD(self.gvt[trtp.work - 1]) + \
            ASD(taxamt.work))
        return (acno, trdt, trtp, ref1, batch, debit, credit, taxamt, detail,
            taxind, name)
    ...
    def typeHeading(self):
        ...
        self.fpdf.drawText("%-7s %7s %-10s %3s" % \
            ("Batch", batch, "    Type", crtrtp[self.trtp - 1][1]))
        self.fpdf.drawText()
        if self.totsonly == "Y":
            ...
            self.fpdf.drawText("%-7s %-30s %-9s %-10s %-15s %-15s %-15s %-1s "\
                "%-30s" % ("Acc-Num", "Name", "Reference", "   Date",
                "        Debits", "       Credits", "    Tax-Amount", "T",
                "Remarks"))
        self.fpdf.underLine(txt=self.head)
        self.fpdf.setFont()
        self.pglin += 4
    ...
    def batchTotal(self):
        ...
        if self.totsonly == "Y":
            ...
            self.fpdf.drawText()
            self.fpdf.drawText("%-7s %-51s %15s %15s %15s" % (" ",
