...
def getColors(style, scheme):
    def make_lighter(hex_color, factor=1.5):
        """Multiplies RGB values to make them lighter."""
        hex_color = hex_color.lstrip('#')
        r, g, b = [int(hex_color[i:i+2], 16) for i in (0, 2, 4)]
        # Multiply and cap at 255
    ...
    if scheme == "R":
        color = (
            ("#ffffff", "#a40204"),
            ("#ffffff", "#a40204"),
            ("#ffffff", make_lighter("#a40204")),
            ("#000000", "#ffffff"))
    elif scheme == "G":
        color = (
            ("#ffffff", "#00883b"),
            ("#ffffff", "#00883b"),
            ("#ffffff", make_lighter("#00883b")),
            ("#000000", "#ffffff"))
    elif scheme == "B":
        color = (
            ("#ffffff", "#0058ff"),
            ("#ffffff", "#0058ff"),
            ("#ffffff", make_lighter("#0058ff")),
            ("#000000", "#ffffff"))
    else:
        # Defaults
        widgets = {
...
def getPrgPath():
    ...
    if getattr(sys, "frozen", False):
        ...
        if sys.platform == "win32":
            ...
            chk = "ms0000"
        while True:
            if os.path.exists(os.path.join(pth, chk)):
                break
...
def loadRcFile(rcfile=None, default=False):
    ...
    if FITZ:
        ...
        win = os.path.join(getPrgPath()[0], "uty", "SumatraPDF.exe")
