...
class rc3010(object):
    ...
    def mainProcess(self):
        self.tit = ("%03i %s" % (self.opts["conum"], self.opts["conam"]),
            "Rental Owners Audit Trail (%s)" % self.__class__.__name__)
        data = ["All Types"]
        for typ in rttrtp:
            data.append(typ[1])
        btt = {
        ...
        btm = {
            ...
            "cols": (
                ("rot_batch", "", 0, "Bat-Num"),
                ("rot_type", ("xx", rttrtp), 20, "Type"),
                ("rot_curdt", "", 0, "Cur-Dat")),
            "where": [],
            "group": "rot_batch, rot_type, rot_curdt",
            "order": "rot_type, rot_curdt, rot_batch"}
    ...
    def doBatTyp(self, frt, pag, r, c, p, i, w):
        ...
        self.whr = [("rot_cono", "=", self.opts["conum"])]
        if self.ptyp == "F":
            self.whr.append(("rot_curdt", "between",
            self.sperw, self.eperw))
        else:
            self.whr.append(("rot_capdt", "between", self.sdatw, self.edatw))
        if w:
            self.whr.append(("rot_type", "=", w))
        self.df.topf[pag][i+1][8]["where"] = self.whr

    def doBatNum(self, frt, pag, r, c, p, i, w):
        self.batch = w
        if self.batch:
            self.whr.append(("rot_batch", "=", self.batch))
    ...
    def printReport(self, recs):
        ...
        for num, dat in enumerate(recs):
            ...
            batch = CCD(dat[self.col.index("rot_batch")], "Na", 7)
            refno = CCD(dat[self.col.index("rot_refno")], "Na", 9)
            trdt = CCD(dat[self.col.index("rot_trdt")], "D1", 10)
            desc = CCD(dat[self.col.index("rot_desc")], "NA", 30)
            tramt = CCD(dat[self.col.index("rot_tramt")], "SD", 13.2)
            taxamt = CCD(dat[self.col.index("rot_taxamt")], "SD", 13.2)
            taxind = CCD(dat[self.col.index("rot_taxind")], "NA", 1)
            if not self.trtp:
                self.trtp = trtp.work
            ...
            if self.pglin > self.fpdf.lpp:
                self.pageHeading()
            if self.totsonly != "Y":
                self.fpdf.drawText("%s %s %s %s %s %s %s %s" % (acno.disp,
                    refno.disp, trdt.disp, desc.disp, tramt.disp, taxamt.disp,
    ...
    def typeHeading(self):
        ...
        if self.totsonly == "Y":
            self.fpdf.drawText(
            "%-48s %-13s %-13s" % ("Details", "      Amount", "  Tax-Amount"))
        else:
            self.fpdf.drawText(
            "%-7s %-9s %-10s %-30s %-13s %-13s %-1s %-30s" % ("Acc-Num",
            "Reference", "   Date", "Remarks", "      Amount", "  Tax-Amount",
            "I", "Owner"))
        self.fpdf.underLine(txt=self.head)
        self.pglin += 4
        self.fpdf.setFont()
    ...
    def batchTotal(self):
        ...
        if self.totsonly == "Y":
            ...
            self.fpdf.drawText()
            self.pglin += 1
            self.fpdf.drawText("%-28s %-30s %13s %13s" % (" ",
                "Batch " + self.batch + " Totals", j.disp, k.disp))
        self.pglin += 1
        if self.totsonly != "Y":
            self.fpdf.drawText()
